...
namespace Planned.Services
{
    ...
    public class GetIdentityServiceRequestHandler : ServiceRequestHandler
    {
        ...
        public override object HandleRequest(DataControllerService service, JObject args)
        {
            ...
            {
                UserTicket ticket = ApplicationServices.Current.CreateTicket(user);
                ...
                res.Clear();
                res.Write("<html><body><script>window.CloudOnTime.invoke(\'addidentity\', \'");
    ...
    public class GetManifestServiceRequestHandler : ServiceRequestHandler
    {
        ...
        public override object HandleRequest(DataControllerService service, JObject args)
        {
            ...
            if (HttpContext.Current.Request.IsAuthenticated)
            {
                ...
                files.Add(new ManifestFile("touch-settings.json"));
        ...
        void AddSiteFiles(List<ManifestFile> files, string siteFolder)
        {
            ...
            foreach (string file in Directory.EnumerateFiles(Path.Combine(rootPath, siteFolder), "*.*", SearchOption.AllDirectories))
            {
                ...
                if (!(m.Success) || (m.Groups["Culture"].Value == Thread.CurrentThread.CurrentCulture.Name))
                	files.Add(new ManifestFile(file.Substring(rootPath.Length)));
            }
        }
    ...
    public class GetManifestFileServiceRequestHandler : ServiceRequestHandler
    {
        ...
        public override object HandleRequest(DataControllerService service, JObject args)
        {
            ...
            if (requestedFile == "js\\host\\config.js")
            	HttpContext.Current.Response.Write(GetManifestServiceRequestHandler.GetConfigJS());
            else
            {
                string p = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, requestedFile);
                if (File.Exists(p))
    ...
    public class ApplicationServicesBase
    {
        ...
        public virtual JObject DefaultSettings
        {
            get
            {
                if (_defaultSettings == null)
                {
                    ...
                    EnsureJsonProperty(_defaultSettings, "appName", "Portfolio Manager");
                    ...
                    EnsureJsonProperty(ui, "actions.promote", false);
        ...
        public virtual Image UserPictureFromCMS(MembershipUser user)
        {
            if (IsSiteContentEnabled)
            {
                SiteContentFileList list = ReadSiteContent("sys/users", (user.UserName + ".%"));
